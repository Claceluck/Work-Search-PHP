<main>
	<!-- Основной контент -->
	<div class="conteiner info_buton">
		<h4 class="logo_2"> Эта страница поможет вам найти работу или стажировку в разных странах мира!</h4>
	</div>
	<!-- Кнопка перехода на анкету -->
	<div class="conteiner button_block">
		<a class="start_button" href="/registration">Начать поиск</a>
	</div>
	<!-- Блок статитстики -->
	<h3 class="conteiner">Статистика</h3>
	<div class="conteiner stat_block stat_block_xm stat_block_xs">
			<div class="canvas">
				<div>
				<p class="title">Наиболее востребованные профессии за рубежом:</p>
				<ul class="caption-list">
				<li class="caption-item">Врачи</li>
				<li class="caption-item">Водители</li>
				<li class="caption-item">IT-Персонал</li>
				<li class="caption-item">Строители</li>
				<li class="caption-item">Медицинский персонал и фармацевты</li>
				<li class="caption-item">Инженеры</li>
				<li class="caption-item">Повара</li>
				</ul>
			</div>
			</div>
			<div>
			<svg class="canvas" viewBox="0 0 50 50">
				<circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
				<circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
				<circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
				<circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
				<circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
				<circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
				<circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
			</svg>
			</div>
	</div>
	<!-- Блок историй людей -->
	<h3 class="story_header">Новости портала</h3>
	<div class="conteiner story_block story_block_xm story_block_xs">
		<div class="story_block_1 story_block_1_xs">
			<img class="story_img" src="/static/img/ny.jpeg" alt="#">
			<div>
			<h3>Как найти работу в США</h3>
			<h5>Автор: <a class="avtor" href="#">Екатерина Гаврилова</a> | 04.12.2019</h5>
			<p>Я искала работу два с половиной месяца. Мне говорили, что этот процесс займёт не менее полугода, поэтому я счастлива, что оффер на позицию Senior Product Manager от Zello случился раньше. Эта компания соответствует всем критериям, которые я выдвигала к работодателю...</p>
				<a class="read_later" href="#"> <span>Читать дальше...</span></a>
			</div>
		</div>
		<div class="story_block_1 story_block_1_xs">
			<img class="story_img" src="/static/img/ispan.jpg" alt="#">
			<div>
			<h3>Собираем апельсины в Испании</h3>
			<h5>Автор: <a class="avtor" href="#">Егор Иванов</a> | 04.12.2019</h5>
			<p>Поспевают цитрусовые в Испании круглый год: только собрали первый урожай, а за ним уже на подходе следующий. Так как работать предстоит в поле, то рассчитывать на комфорт не приходится. Тем, кто не знает, как происходит сбор апельсинов в Испании, понять весь процесс поможет следующая информация..</p>
			<a class="read_later" href="#"> <span>Читать дальше...</span></a>
			</div>
		</div>
	</div>
</main>
